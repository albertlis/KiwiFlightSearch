...
def send_mail(print_info: str) -> None:
    ...
    yag.send(to=os.getenv("DST_MAIL"), subject=email_subject, contents="Loty Kiwi", attachments=[zip_buffer])
...
def main() -> None:
    ...
    print_info = flights_processor.process_flights_info(
        flights_data, scrape_errors=scrape_errors, lookup_errors=lookup_errors
    )

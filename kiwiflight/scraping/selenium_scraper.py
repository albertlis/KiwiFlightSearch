"""Selenium based scraper (legacy / fallback) wrapped into package interface.

Thin wrapper around existing driver + kiwi_scrapper logic kept for compatibility.
"""
import json
import logging
import pickle
...
class SeleniumScraper(Driver):
    def __init__(self, start_month: str, end_month: str, start_iata_airports: list[str]):
        ...
        self.price_span_locator = (self.price_span_locator if hasattr(self, 'price_span_locator') else (None, None))
    ...
    def _gather_route(self, wait: WebDriverWait, start_code: str, start_name: str, dst_code: str, dst_name: str):
        flights: list[FlightInfo] = []
        # Reuse logic from legacy scraper would go here; simplified placeholder to keep interface.
        # Proper migration can be added later if Selenium path is still required.
        return flights

    def webscrap_flights(self):  # keep naming
        driver, wait = self.get_driver()
        driver.get(self.url)
        self.setup_main_page(wait)
        poland_to_anywhere: list[FlightInfo] = []
        anywhere_to_poland: list[FlightInfo] = []

...
class BaseFlightProcessor(ABC):
    ...
    def get_flight_time(self, flight: FlightInfo, flight_type: str = "departures") -> time | None:
        ...
        for flight_info in timetable:
            if (
                flight_info.start_date <= flight.date <= flight_info.end_date
                and flight.date.weekday() in flight_info.weekdays
            ):
                return flight_info.start_time
        logging.debug(
            f"No timetable flight match. Probably indirect flight for {flight} ({flight.date.strftime('%A')})"
        )
        return None
